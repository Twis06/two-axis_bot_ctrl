...
- [ ] Validate a closed-loop prediction using the actual frozen controller and information mode, including delay/filter effects. **Not established:** existing predictions are ideal component arithmetic. R2 must report this gap explicitly; a new prospective study requires a separate frozen protocol before execution. *(Declared open limitation in the final submission: no prospective closed-loop prediction was registered or tested.)*
