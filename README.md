...
- **Checking a reproduction:** reproduce in a separate copy (for example `git archive HEAD | tar -x -C /tmp/copy`, then `uv run python run_all.py` there), then run `uv run python tools/compare_evidence.py /tmp/copy/report` from this checkout. It compares the copy with the published `report/`; running it after an in-place reproduction would compare the directory with itself. On macOS arm64 expect exact agreement; on x86-64, values within about 1e-11 and different Monte Carlo run_ids, because NumPy rounds random draws differently across CPUs (see `report/packets/R4.md`).
...
- ✅ Corrective packets R1–R5 done; [R6](report/packets/R6.md) round 2 passed with minor issues on commit `961b6f2`. Subsequent editorial changes require separate verification.
