...
| 3 · Task 5 inference | Resolved in the memo, HTML and `task5.md`: the selected post hoc residual-peak metric did not resolve the predicted effect, and the Run B inference is scoped. A prospective closed-loop prediction was carried out afterwards and supported (see `docs/plans/task5-prospective-protocol.md`). |
...
| 8–9 · progress and release status | Resolved: B/C path-clock progress is qualified; test counts and review status are current (278 tests at the final commit). |
