...
def main():
    ...
    head = ['# Task 2 — regenerated evidence', '',
            ...
            f"git `{(fp['git'] or {}).get('commit', '?')[:12]}`, baseline sources "
            f"{'clean' if not (fp['git'] or {}).get('dirty_sources') else 'MODIFIED: ' + ', '.join(fp['git']['dirty_sources'])}"
            f" (uncommitted files in the wider set: {', '.join(fp['run_set_git'].get('dirty_sources') or []) or 'none'}); "
