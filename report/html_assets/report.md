...
**Why acquisition matters:** the published first-usable times show only 4 of 50 held-out cases usable before the 9 s test phase, 20 first usable during it, 13 after it and 13 never usable. These counts come from the published first-usable times. An exploratory replay, whose wrapper script is not in the repository, suggests that the bottleneck is eligible dwell count and angle/feature coverage rather than worker latency (4–8 ms once those gates pass); treat that as a hypothesis for the next registered test, not as reproduced evidence. A usable snapshot can still have too little applied correction because coefficients ramp and the controller uses bumpless transfer. See the [per-run availability review](../docs/plans/task3-availability-review.md).
...
Five-seed medians, except counts. Net progress is path-clock progress, not a measured speed ratio. B/C command a stationary 0° roll hold while yaw oscillates, so their 100%/97% clock values are not roll distance delivered; Run C's yaw request is reduced to 0.87–0.89× (median 0.88×) during ramp-up in every one of its five runs, so C's delivered yaw is smaller than requested. “Tracked” requires ≥95% progress, ≤2° request-window path RMS, and no rejection, suspension or tracking fault; these are project thresholds. D/E's large original-time error exposes sacrificed timing. Their small governed error is not delivery of the original request. E has 146 saturation entries despite 5.5% occupancy.
...
| 2 · Guarded low-speed rotation | Supported axis initially within ±5° at 2.4 A. Measure back-EMF Ke from voltage/current/speed where Ke·q̇ is at least comparable to the IR drop (or by back-driving the unpowered axis), and resolve the bus-to-phase/PWM convention. | Stop for unexpected motion/voltage. Re-run model and margins if Ke or available voltage differs before broader moves. |
...
| 7 · Payload and envelope | D-like shifted load at reduced speed, then the loaded finite move. | Every request is completed within ≤2° RMS / ≤5° peak, or visibly reshaped/rejected with the reason logged; no current-limit bypass; no unexplained fault. |
