...
- **Test counts:** the final suite has **266 tests**. An earlier snapshot at `dff8efd` passed 255 tests; that count is historical. The focused suites cover the estimator, adapter contract, phase-aware scorer and adoption gate. Passing tests show the code does what the tests check; they do not validate the experiment design.
